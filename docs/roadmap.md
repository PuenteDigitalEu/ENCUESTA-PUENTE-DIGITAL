...
|---|---------|--------|--------|
| 0.1 | Cerrar los 8 documentos de `docs/` con el usuario. | — | Los 8 en estado distinto de "Borrador para validar". |
...
| 0.3 | Escribir `guion-entrevista.md` + `instrucciones-sistema.md` + `instrucciones-rubrica.md` (raíz del repo) y confirmarlos. | Base de `M-03`, `M-04` | El guion cubre los 8 bloques; el usuario da el visto bueno. |
| 0.4 | Diseñar la rúbrica: niveles de `nivel_preparacion`, catálogo de casos de uso, fórmula impacto × viabilidad, umbrales. | Base de `M-05` | Documentada en `instrucciones-rubrica.md` / `docs/data-model.md`; enum `nivel_preparacion` fijado. |
| 0.5 | Reescribir `supabase/migrations/001` con el esquema de `docs/data-model.md` (8 tablas, renombrados, `on delete cascade`). | Base de `M-09` | `node scripts/verificar-persistencia.mjs` en verde contra el esquema nuevo. |
...
| 0.7 | Resolver el contraste del acento coral (`design-system.md` §4) y actualizar el token. | `RNF-05` (parte) | Acento con texto blanco pasa AA. |
| 0.8 | (Si se aprueba) Ampliar CI: `pnpm lint` + `pnpm test` + `pnpm build` además de la cobertura. | `RNF-06` (soporte) | Workflow nuevo pasa en un PR de prueba. |
...
sin estado en servidor. `ConsentScreen` con el copy nuevo.
**Puerta:** tests de ruta en verde; recorrido manual: consentir → 8 bloques → emite la ficha de
cierre; sin consentir no se crea nada.
...
contacto → respuestas → resultado → diagnóstico.
**Puerta:** sin los 4 campos no hay diagnóstico; con ellos, se persiste todo y se muestra; el
diagnóstico cita el nivel y los casos de la rúbrica (muestreo manual).
...
fecha (empresa, contacto, badge de nivel, nº de casos). Detalle: contacto, respuestas por los 8
bloques, diagnóstico como lo vio el visitante.
...
| Envolver el cierre en una RPC transaccional de Postgres. | Riesgo asumido en `architecture.md` §7 / `data-model.md` §8 |
...
- **Instrumentar el embudo** de `business.md`: encuestas iniciadas / completadas / tasa de
  completado / leads cualificados / coste de IA por encuesta. Visible en el panel o como export.
