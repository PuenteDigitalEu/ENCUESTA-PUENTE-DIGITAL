...
Pendiente de definir con el usuario (ver "Estado del proyecto y arranque" arriba). La idea de
partida: una landing de acceso cerrado (URL genérica, sin cuentas) con un agente conversacional
que hace a un responsable de una pyme o autónomo una encuesta guiada sobre sus procesos, y al
cerrarla le devuelve en el propio chat un diagnóstico de preparación para IA y una lista
...
- No le pidas al modelo de lenguaje que calcule el resultado de la encuesta (nivel de preparación,
  puntuaciones, ranking de casos de uso). Ese cálculo vive en código determinista y testeado
  (`src/lib/` — el clon trae `src/lib/motor/` como referencia a reescribir); Claude conduce la
  conversación y redacta el diagnóstico narrativo a partir de esos valores ya calculados.
- No inventes ni completes una respuesta que el usuario de la encuesta no ha dado. Si falta un dato
  que la rúbrica necesita, el diagnóstico se queda incompleto y lo dice — no se rellena el hueco con
  una suposición.
- El clon arrastra `src/app/api/chat/` ya escrito para el flujo de la asesoría. Antes de tocarlo,
  redefine el guion de la entrevista y la rúbrica en `docs/` y confírmalos con el usuario; no
  adaptes el código a ciegas.
